<?php 
	class yatcoConnect_ApiToBossForBoatdeck {
		...
		public function get_dummy_products(){
			...
			$filename = ABSPATH .'/response.json';
			...
			if (file_exists($filename)) {
				...
				$data = json_decode($json_string, true); // use true for associative array

				// Optional: check for errors
				if (json_last_error() === JSON_ERROR_NONE) {
					return $data; // Use the data as needed
				} else {
		...
		public function get_boatdeck_products(){

			$products = $this->call_api('GET', '/boatdeck/product/search');

			//$products = $this->get_dummy_products();
